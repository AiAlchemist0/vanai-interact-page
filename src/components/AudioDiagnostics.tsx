      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={audioValidation.validateAllAudioFiles} disabled={audioValidation.isValidating}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="grid gap-3">
              ...
              return <div key={songId} className="p-3 border rounded-lg">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex gap-2">
                        {validationStatus?.isValid ? <Badge variant="outline" className="text-green-700">
                            <CheckCircle className="h-3 w-3 mr-1" />
                            Valid
                          </Badge> : validationStatus?.error ? <Badge variant="destructive">
                            <AlertTriangle className="h-3 w-3 mr-1" />
                            Failed
                          </Badge> : <Badge variant="outline">
                            Not Tested
                          </Badge>}
                        <Button size="sm" variant="outline" onClick={() => audioValidation.validateSingleAudio(songId)}>
                          <Play className="h-3 w-3 mr-1" />
                          Test
                        </Button>
                      </div>
                    </div>
                    {validationStatus?.error && <div className="text-xs text-red-600 bg-red-50 p-2 rounded">
                        Error: {validationStatus.error}
                      </div>}
                    {validationStatus?.duration && <div className="text-xs text-green-600">
          ...
          {audioValidation.lastValidationTime && <div className="text-xs text-muted-foreground border-t pt-2">
